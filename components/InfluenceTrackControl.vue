<template>
  <div class="space-y-2">
    <div 
      ...
      :key="house.id"
      class="flex items-center justify-between p-3 bg-background border rounded-lg"
...
const moveUp = (index: number) => {
  ...
  [newHouses[index - 1], newHouses[index]] = [newHouses[index], newHouses[index - 1]];
...
const moveDown = (index: number) => {
  ...
  const newHouses = [...localHouses.value];
  [newHouses[index], newHouses[index + 1]] = [newHouses[index + 1], newHouses[index]];
