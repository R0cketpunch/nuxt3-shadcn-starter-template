...
import {
  ...
  getStartingFiefdomsOrder,
  getStartingKingsCourtOrder,
  STARTING_WILDLING_THREAT,
} from "~/types/game";
...
export const useGameState = () => {
  ...
  watch(
    gameState,
    (newState) => {
      gameSounds.processStateChange(newState);
    },
    { deep: true }
  );
  ...
  const listenForStateChanges = () => {
    if (typeof window !== "undefined" && !listenersInitialized) {
      ...
      realtimeSync.onTimerAction(
        ({ action, duration, timeAdjustment, timestamp, serverTime }) => {
  ...
  const initializeGame = (selectedHouses: House[]) => {
    ...
    const ironThroneOrder = startingIronThrone.map((predefinedHouse) => {
      const selectedHouse = selectedHouses.find(
        (h) => h.id === predefinedHouse.id
      );
  ...
  const nextPhase = () => {
    const wasRoundAdvance =
      gameState.value.currentPhase.id === "action" &&
      gameState.value.currentRound < MAX_ROUNDS;
    ...
    if (gameState.value.currentPhase.id === "action") {
      ...
      if (gameState.value.currentRound < MAX_ROUNDS) {
        ...
        gameState.value.currentSubPhase =
          gameState.value.currentRound === 1
            ? undefined
            : WESTEROS_SUBPHASES[0];
        gameState.value.currentPlayerIndex = 0;
      } else {
        // Game has ended (reached max rounds) - sound will be handled by watcher
      }
  ...
  const nextSubPhase = () => {
    ...
    if (gameState.value.currentPhase.id === "westeros") {
      const currentSubPhaseIndex = WESTEROS_SUBPHASES.findIndex(
        (sp) => sp.id === gameState.value.currentSubPhase!.id
      );

      if (currentSubPhaseIndex < WESTEROS_SUBPHASES.length - 1) {
        gameState.value.currentSubPhase =
          WESTEROS_SUBPHASES[currentSubPhaseIndex + 1];
        gameState.value.currentPlayerIndex = 0;
      } else {
      ...
      const currentSubPhaseIndex = PLANNING_SUBPHASES.findIndex(
        (sp) => sp.id === gameState.value.currentSubPhase!.id
      );

      if (currentSubPhaseIndex < PLANNING_SUBPHASES.length - 1) {
        gameState.value.currentSubPhase =
          PLANNING_SUBPHASES[currentSubPhaseIndex + 1];
        gameState.value.currentPlayerIndex = 0;
      } else {
      ...
      const currentSubPhaseIndex = ACTION_SUBPHASES.findIndex(
        (sp) => sp.id === gameState.value.currentSubPhase!.id
      );

      if (currentSubPhaseIndex < ACTION_SUBPHASES.length - 1) {
        gameState.value.currentSubPhase =
          ACTION_SUBPHASES[currentSubPhaseIndex + 1];
        gameState.value.currentPlayerIndex = 0;
      } else {
        // End of action phase
        nextPhase();
  ...
  const nextPlayer = () => {
    ...
    gameState.value.currentPlayerIndex =
      ...
      gameState.value.ironThroneOrder.length;
    broadcastStateChange();
  };
  ...
  const getCurrentPlayer = (): House | null => {
    return (
      gameState.value.ironThroneOrder[gameState.value.currentPlayerIndex] ||
      null
    );
  };
  ...
  const getNextAction = (): {
    action: "nextPlayer" | "nextSubPhase" | "nextPhase" | "complete";
    label: string;
  } => {
    if (isGameComplete()) {
    ...
    if (currentSubPhase) {
      ...
      const currentSubPhaseIndex = subPhases.findIndex(
